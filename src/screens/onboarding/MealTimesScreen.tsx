...
const MealTimesScreen: React.FC<OnboardingScreenProps<'MealTimes'>> = ({ navigation }) => {
  // Get the content for this screen from onboarding content
  const content = onboardingContent.find(item => item.id === 'mealTimes')!;
  const { updatePreference, preferences } = useOnboarding();
  const [selected, setSelected] = useState<string[]>(preferences.mealTimes || []);
  ...
  const handleContinue = () => {
    updatePreference('mealTimes', selected);
    navigation.navigate('CookingSkill');
  };
      ...
      <View style={styles.buttonContainer}>
        {content.allowSkip && (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('CookingSkill')}
          >
